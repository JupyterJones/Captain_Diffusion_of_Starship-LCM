...
# CONFIG & PATHS
# ============================================
SD_SERVER_URL = os.environ.get("SD_SERVER_URL", "http://127.0.0.1:5000")
...
# Ensure static/porthole exists
OUTPUT_DIR = os.path.join(BASE_DIR, "static", "porthole")
STATE_FILE = os.path.join(OUTPUT_DIR, "porthole.json")
LOG_FILE_PATH = os.path.join(OUTPUT_DIR, "porthole.txt")
...
def listen_to_comfy():
    """Stubbed WebSocket listener - Good SD Server operates synchronously via HTTP POST."""
    pass
...
# ============================================================
try:
    res = requests.get(f"{SD_SERVER_URL}/api/models", timeout=10).json()
    MODELS = res.get("models", [])
    LORAS = ["None"] + res.get("loras", [])
except Exception as e:
    logit(f"Error fetching models from {SD_SERVER_URL}: {e}")
    MODELS, LORAS = [], ["None"]
...
def draw_metadata_caption(img, frame_idx, total_frames, metadata, curr_zoom, curr_pan_x, curr_pan_y):
    try:
        ...
        return img
'''
def apply_border(img, border_path="static/porthole.png"):
    """
...
def render_video(resume=False):
    ...
    if running: return
    logit("ENGINE STARTED: Entering render loop (Good SD Server backend).")
    ...
    if current_frame > 0:
        prev = os.path.join(OUTPUT_DIR, f"frame_{current_frame-1:03d}.png")
        if os.path.exists(prev):
            try:
                with open(prev, "rb") as f:
                    last_image_b64 = base64.b64encode(f.read()).decode('utf-8')
            except Exception as e:
                logit(f"Error loading prev image: {e}")
    ...
    try:
        while current_frame < frames_current:
            ...
            payload = {
                ...
                "seed": active_s,
                "model": model_name,
                "lora1": lora1_name if lora1_name != "None" else "",
                "lora1_weight": 0.8,
                "lora2": lora2_name if lora2_name != "None" else "",
                "lora2_weight": 0.8
            }
            
            if last_image_b64 and current_frame > 0 and active_d < 1.0:
                payload["init_image"] = last_image_b64
                payload["strength"] = active_d

            try:
                logit(f"Sending generation request (Frame {current_frame}) to {SD_SERVER_URL}")
                resp = requests.post(f"{SD_SERVER_URL}/api/generate", json=payload, timeout=1200).json()
                if "error" in resp:
                    logit(f"SD Server error (Frame {current_frame}): {resp['error']}")
                    time.sleep(3)
                    continue

                data_url = resp.get("imageUrl", "")
                if not data_url:
                    logit(f"No imageUrl returned for frame {current_frame}")
                    time.sleep(3)
                    continue

                b64_str = data_url.split(",")[1] if "," in data_url else data_url
                raw_bytes = base64.b64decode(b64_str)
                img = Image.open(io.BytesIO(raw_bytes)).convert("RGB")
            except Exception as e:
                logit(f"Error generating frame {current_frame}: {e}")
                time.sleep(3)
                continue
            ...
            try:
                ...
                meta = {
                    ...
                    "steps": default_steps,
                    "cfg": default_cfg,
                    "denoise": active_d if last_image_b64 else 1.0
                }

                # 1. Apply zoom AND spaceship movement (BASE for both server and disk)
                ...
                # 2. FEEDBACK IMAGE FOR NEXT FRAME
                # Apply feedback loop stabilization (color, contrast, sharpness)
                ...
                buffered = io.BytesIO()
                feedback_img.save(buffered, format="PNG")
                last_image_b64 = base64.b64encode(buffered.getvalue()).decode('utf-8')
...
def teleport():
    ...
    try:
        ...
        with state_lock:
            teleport_image = b64_str
        
        logit("Teleport image set successfully.")
        return jsonify({"status": "ok", "filename": "teleport.png"})
    except Exception as e:
...
<body>
    <div class="column left">
        <h3>FlaskArchitect's Epochporthole Engine Config</h3>
        ...
        <div style="display:flex; gap:5px;">
            ...
            <div style="flex:1;">
                <label>CFG</label>
                <input type="number" id="cfg" value="5.5" step="0.1">
            </div>
        </div>
        ...
        <div style="display:flex; gap:5px; margin-top:5px;">
            <div style="flex:1;">
                <label>Denoise</label>
                <input type="number" id="denoise" step="0.01" value="0.35">
            </div>
...
<button class="btn-green" onclick="window.open('/media', '_blank')">
    ...
    <script>
        ...
        function ctrl(a,b){ fetch('/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:a, prompt:document.getElementById('prompt').value})}).then(()=>showFeedback("Action Sent")); }
        ...
        setInterval(()=>{
            fetch('/status').then(r=>r.json()).then(d=>{
                ...
                document.getElementById('status_text').innerText = statusMsg + " " + d.frame + "/" + d.total;
                
                if(d.frame>0) document.getElementById('preview').src='/latest_frame?t='+Date.now();
                if(d.history) document.getElementById('thumb_strip').innerHTML = d.history.map(i=>`<img src="/static/porthole/${i}?t=${Date.now()}">`).join('');
...
SHARED_PATH = os.path.join(BASE_DIR, "static", "porthole")
...
{% for v in videos %}
<div class="box">
<video controls src="{{ url_for('static', filename='porthole/' + v) }}"></video><br>
...
{% for img in images %}
<div class="box">
<img src="{{ url_for('static', filename='porthole/' + img) }}"><br>
