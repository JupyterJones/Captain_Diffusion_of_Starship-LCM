...
OUTPUT_DIR = os.path.join(BASE_DIR, "static", "spaceexplorer")
STATE_FILE = os.path.join(OUTPUT_DIR, "spaceexplorer.json")
LOG_FILE_PATH = os.path.join(OUTPUT_DIR, "spaceexplorer.txt")
...
negative_prompt = "bad anatomy, extra limbs, blurry, low quality, watermark, text, deformed, mutated hands, worst quality, ugly face, pure black background"

injection_lines = []
MAX_LINES = 5
keyframes = {
    "0": {
        "prompt": "A majestic flying saucer blasting off from a futuristic launchpad on Earth, giant plumes of fire and smoke, cinematic lighting, photorealistic, 8k resolution, dramatic clouds in the atmosphere",
        "denoise": 1.0,
        "seed_offset": 0
    },
    "50": {
        "prompt": "A majestic flying saucer in space and is orbiting the blue Earth, sun glinting off solar panels, deep black space, stars, realistic reflections, high detail",
        "denoise": 0.35,
        "seed_offset": 5
    },
    "100": {
        "prompt": "A majestic flying saucer cruising through a vast starfield, nebula gas clouds glowing blue and purple, distant galaxies, cosmic scale, cinematic sci-fi",
        "denoise": 0.32,
        "seed_offset": 12
    },
    "135": {
    "prompt": "A majestic flying saucer cruising through a vast starfield, nebula gas clouds glowing blue and purple, distant galaxies, cosmic scale, cinematic sci-fi, earth is not visible",
    "denoise": 0.32,
    "seed_offset": 14
    },
    "155": {
    "prompt": "A majestic flying saucer cruising through a vast starfield, approaching an asteroid ",
    "denoise": 0.32,
    "seed_offset": 19
    }    
}
...
# ============================================================
def create_movie_from_frames(output_filename="production_june22.mp4"):
    """
...
def generate_story():
    try:
        ...
        if not files:
            return jsonify({"error": "No generated frames found in static/spaceexplorer/ to process."}), 400
...
def teleport():
    ...
    try:
        ...
        # Save locally in the output/spaceexplorer folder
...
# ============================================================
HTML_UI = """
<!DOCTYPE html><html><head><title>SpaceExplorer PIL AI Director</title>
<style>
    body { 
        margin: 0; 
...
<body>
    <div class="column left">
        <h3>FlaskArchitect's SpaceExplorer Engine Config</h3>
    ...
    <script>
        ...
        setInterval(()=>{
            fetch('/status').then(r=>r.json()).then(d=>{
                ...
                if(d.history) document.getElementById('thumb_strip').innerHTML = d.history.map(i=>`<img src="/static/spaceexplorer/${i}?t=${Date.now()}">`).join('');
...
SHARED_PATH = os.path.join(BASE_DIR, "static", "spaceexplorer")
...
{% for v in videos %}
<div class="box">
<video controls src="{{ url_for('static', filename='spaceexplorer/' + v) }}"></video><br>
...
{% for img in images %}
<div class="box">
<img src="{{ url_for('static', filename='spaceexplorer/' + img) }}"><br>
...
<audio controls style="width:250px; display:block; margin: 0 auto 8px auto;">
    <source src="{{ url_for('static', filename='spaceexplorer/' + mp3) }}" type="audio/mpeg">
</audio>
...
def compile_story_movie():
    ...
    if not wav_files:
        return "No narration WAV files found in static/spaceexplorer/. Please generate your story first.", 400
