...
class SDRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/api/select-model":
            ...
            width = (int(data.get("width", 512)) // 8) * 8
            height = (int(data.get("height", 512)) // 8) * 8
            ...
            # Parse init_image and strength for img2img
            init_image_data = data.get("init_image", "")
            strength = float(data.get("strength", data.get("denoise", 0.35)))
            ...
            if init_image_data:
                try:
                    ...
                    print(f"    [!] Warning: Failed to process init_image: {img_err}")
                    init_image = None
            ...
            if init_image is not None:
                print(f"    - Img2Img Mode Active | Strength (Denoise): {strength}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="/media/jack/9930-14E11/ComfyUI/models/checkpoints/ghibliLikeLCM_v10.safetensors")#type=str, required=True, help="Path to local checkpoint file or a directory of checkpoints")
